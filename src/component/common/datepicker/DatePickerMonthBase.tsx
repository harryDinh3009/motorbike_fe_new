...
interface DatePickerMonthBaseProps extends DatePickerProps {
  id?: string;
  required?: boolean;
  value?: string;
...
const DatePickerMonthBase: React.FC<DatePickerMonthBaseProps> = ({
  id,
  required = false,
  value,
  onChange,
  ...
  useEffect(() => {
    setInternalValue(value ? dayjs(value, "YYYY-MM") : null);
  }, [value]);
  ...
  const handleChange = (date: Dayjs | null) => {
    const dateString = date ? date.format("YYYY-MM") : null;
  ...
  return (
    <>
      ...
      <input
        type="hidden"
        id={id}
        value={internalValue ? internalValue.format("YYYY-MM") : ""}
        required={required}
      />
    </>
  );
...
export default DatePickerMonthBase;
