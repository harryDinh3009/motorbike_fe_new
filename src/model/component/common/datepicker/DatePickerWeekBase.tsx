...
interface DatePickerWeekBaseProps extends DatePickerProps {
  id?: string;
  required?: boolean;
  value?: string;
...
const DatePickerWeekBase: React.FC<DatePickerWeekBaseProps> = ({
  id,
  required = false,
  value,
  onChange,
  ...
  const [internalValue, setInternalValue] = useState<Dayjs | null>(
    value ? dayjs(value, "YYYY-[W]WW") : null
  );

  useEffect(() => {
    setInternalValue(value ? dayjs(value, "YYYY-[W]WW") : null);
  }, [value]);
  ...
  const handleChange = (date: Dayjs | null) => {
    const dateString = date ? date.format("YYYY-[W]WW") : null;
  ...
  return (
    <>
      ...
      <input
        type="hidden"
        id={id}
        value={internalValue ? internalValue.format("YYYY-[W]WW") : ""}
        required={required}
      />
    </>
  );
...
export default DatePickerWeekBase;
